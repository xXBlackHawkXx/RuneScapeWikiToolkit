...
function mergeSetCookies(existingHeader, setCookies) {
  ...
  for (const rawCookie of setCookies) {
    ...
    const shouldExpire = attributes.some((attribute) => {
      ...
      if (normalizedKey === 'max-age') return Number(attributeValue) <= 0;
    ...
    if (shouldExpire) cookies.delete(name);
...
function getOrCreateSession(request, reply) {
  ...
  if (!sid || !sessions.has(sid)) {
    sid = randomUUID();
    sessions.set(sid, { cookies: '', touchedAt: now });
  }
  ...
  const isSecureCookie = process.env.SESSION_COOKIE_SECURE === 'true' || cookieSameSite === 'none';
...
export function registerMediaWikiRoutes(app) {
  app.route({
    method: ['GET', 'POST'],
    url: '/api/mediawiki',
    handler: async (request, reply) => {
      ...
      const headers = { accept: 'application/json', cookie: session.cookies };
